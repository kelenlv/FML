...
def train_normal(node,Node_List):
...
def train_mutual(node,Node_List):
    node.model.to(node.device).train()
    node.meme.to(node.device).train()
...
#            print(output_local)
#            print('??:\n')
#            print(LogSoftmax(output_local))
#            print('\n')
            
#            print(target)# predict 1-10
            output_meme = node.meme(data)
#            print(output_meme.detach())
...
class Trainer(object):
    ...
    def __call__(self, node,Node_List):
        self.train(node,Node_List)
